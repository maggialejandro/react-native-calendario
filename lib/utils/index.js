...
import type { MonthType, DayType, LocaleType, ViewableItemsType } from '../types';
...
export function monthShouldBeReRendered(
  month: MonthType,
  startDate?: Date,
  endDate?: Date,
): boolean {
  const currentMonth = moment([month.year, month.monthNumber]).toDate();
  const lastDayCurrentMonth = moment([month.year, month.monthNumber]).endOf('month').toDate();

  if (endDate && startDate) {
    return lastDayCurrentMonth >= startDate
