  ...
  return days.map((day: DayType): DayType => {
    ...
    if (!day.isMonthDate) {
      ...
      if (date > lastDayOfMonth) {
        isActive = endDate > lastDayOfMonth && startDate < lastDayOfMonth;
      } else {
        isActive = startDate < firstDayOfMonth && endDate > firstDayOfMonth;
      }
    } else {
...
export function getMonthsList(
  ...
  months.push({
    ...
    days: getDaysOfMonth(monthNumber, year, startDate, endDate, minDate, maxDate, disableRange),
    startDate,
    endDate,
    isVisible,
  });
