...
type CalendarType = {
  ...
  renderDayContent?: (DayType) => ?React.Component<any>,
  extraData: any,
  viewableItemsChanged: (ViewableItemsType) => void,
  calendarListRef?: React$ElementRef<typeof FlatList>,
};
...
export default class Calendar extends React.Component<CalendarType, StateType> {
  ...
  handleViewableItemsChange = (info: ViewableItemsType) => {
    ...
    if (viewableItemsChanged(firstViewableIndex, lastViewableIndex, info)) {
      ...
      lastViewableIndex = lastVisibleItem
  ...
  listRef = (ref: React$ElementRef<typeof FlatList>) => {
  ...
  render() {
    return (
      <FlatList
        ...
        ref={this.props.calendarListRef || this.listRef}
      />
    );
  }
}
