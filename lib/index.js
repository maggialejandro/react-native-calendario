...
export default class Calendar extends React.Component<CalendarType, StateType> {
  ...
  componentWillReceiveProps(nextProps: CalendarType) {
    ...
    if (this.state.startDate !== startDate || this.state.endDate !== endDate) {
      const months: Array<MonthType> = this.recalculateDays(startDate, endDate);
      this.setState({
        startDate,
        endDate,
        months,
  ...
  recalculateDays = (startDate?: Date, endDate?: Date): Array<MonthType> =>
    this.state.months.map((month: MonthType): MonthType => {
      const currentMonth = moment([month.year, month.monthNumber]).toDate();
      const monthHasSelectedDays = monthShouldBeReRendered(month, startDate, endDate)
        || monthShouldBeReRendered(month, this.state.startDate, this.state.endDate);
  ...
  handleViewableItemsChange = (info: ViewableItemsType) => {
    ...
    if (viewableItemsChanged(firstViewableIndex, lastViewableIndex, info)) {
      ...
      console.log('Entra ', firstViewableIndex, lastViewableIndex);
      const months = this.state.months.map((month: MonthType, i: number) => {
        ...
        const days = isVisible && month.days.length ? month.days : getDaysOfMonth(
          monthNumber,
          year,
          startDate,
          endDate,
  ...
  handlePressDay = (date: Date) => {
    ...
    const months: Array<MonthType> = this.recalculateDays(newRange.startDate, newRange.endDate);
